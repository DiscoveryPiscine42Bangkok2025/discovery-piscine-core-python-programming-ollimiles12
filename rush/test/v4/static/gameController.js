...
// --- History & Undo ---
let stateHistory = [];    // stack of {board, turn, ep, cr} snapshots
let moveNotations = [];   // flat list of notation strings: ["e4", "e5", "Nf3", ...]
...
const charToLetter = { 'k': 'K', 'q': 'Q', 'r': 'R', 'b': 'B', 'n': 'N', 'p': '' };

function buildNotation(boardStr, fromR, fromC, toR, toC, isCapture, promotion) {
    const lines = boardStr.split('\n');
    const piece = lines[fromR][fromC];
    const letter = charToLetter[piece.toLowerCase()] || '';
    const from = toSquare(fromR, fromC);
    const to = toSquare(toR, toC);
    const sep = isCapture ? 'x' : '-';
    let notation = letter + from + sep + to;
...
function renderHistory() {
    ...
    if (moveNotations.length === 0) {
        moveList.innerHTML = '<p class="move-empty">No moves yet</p>';
        return;
    }
    let html = '';
...
document.querySelectorAll('.board .square').forEach(sq => {
    ...
    sq.addEventListener('drop', async (e) => {
        ...
        if (!data.valid) {
            ...
            // Record move notation before updating state
            const oldBoard = currentBoardStr;
            const oldLines = oldBoard.split('\n');
            const targetChar = oldLines[dr][dc];
            const isCapture = targetChar !== '.';
            const notation = buildNotation(oldBoard, parseInt(sr), parseInt(sc), dr, dc, isCapture, null);
...
async function triggerBotMove() {
    ...
    try {
        ...
        if (data.from && data.to) {
            ...
            // Record bot move notation
            const oldLines = currentBoardStr.split('\n');
            const targetChar = oldLines[data.to.r][data.to.c];
            const isBotCapture = targetChar !== '.';
            const botNotation = buildNotation(currentBoardStr, data.from.r, data.from.c, data.to.r, data.to.c, isBotCapture, data.promotion);
            moveNotations.push(botNotation);
...
if (undoBtn) {
    undoBtn.addEventListener('click', () => {
        ...
        // If bot is enabled and it's the player's turn, undo 2 states
        // (the bot's move + the player's previous move)
        let undoCount = 1;
        if (botEnabled && stateHistory.length >= 2 && currentTurn !== botColor) {
            undoCount = 2;
        }

        let state;
        for (let i = 0; i < undoCount && stateHistory.length > 0; i++) {
            state = stateHistory.pop();
            moveNotations.pop();
        }

        currentBoardStr = state.board;
        currentTurn = state.turn;
        enPassantTarget = state.ep;
        castlingRights = state.cr;

        renderBoardFromText(currentBoardStr);
        renderHistory();
        resultDiv.textContent = `↩ Undone. ${currentTurn === 'w' ? "White" : "Black"}'s turn.`;
