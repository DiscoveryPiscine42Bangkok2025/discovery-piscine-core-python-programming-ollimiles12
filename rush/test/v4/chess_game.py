...
# FIX 3: Track en passant target square (row, col) of the pawn that can be captured, or None
en_passant_target = None  # (row, col) of the pawn vulnerable to en passant
...
def valid_move(sr, sc, er, ec):
    ...
    if p_type == 'n':
        ...
        if dc == 0 and target == '.' and dr == move_dir:
            ...
            pass
        # FIX 1: En passant capture
        elif abs(dc) == 1 and dr == move_dir and target == '.' and en_passant_target == (sr, ec):
            pass
        else:
            return False
    ...
    # Simulate move (including en passant capture) to check king safety
    orig_s, orig_e = board[sr][sc], board[er][ec]
    ep_captured_piece = '.'
    ep_captured_pos = None

    # FIX 1: Remove the en-passant captured pawn during simulation
    if p_type == 'p' and abs(dc) == 1 and orig_e == '.' and en_passant_target == (sr, ec):
        ep_captured_pos = (sr, ec)
        ep_captured_piece = board[sr][ec]
        board[sr][ec] = '.'
    ...
    if ep_captured_pos:
        board[ep_captured_pos[0]][ep_captured_pos[1]] = ep_captured_piece
...
def is_insufficient_material():
    ...
    if len(pieces) == 3:
        for p, r, c in pieces:
            if p in ('♝', '♗', '♞', '♘'):  # bishop or knight
                return True
    # K+B vs K+B (same color square bishops)
    if len(pieces) == 4:
        bishops = [(p, r, c) for p, r, c in pieces if p in ('♝', '♗')]
        if len(bishops) == 2:
            # Same square color = both on light or both on dark
            if (bishops[0][1] + bishops[0][2]) % 2 == (bishops[1][1] + bishops[1][2]) % 2:
                return True
    return False
